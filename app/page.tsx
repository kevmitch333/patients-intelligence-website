  ...
  return (
    <main id="main-content">
      ...
      <section className="section gap-section"><div className="container">
        <SectionHeading eyebrow="The missing layer" title="Patients have more health information than ever. Care teams need a safe way to receive it." copy="Laboratory platforms, health records, wearables, patient observations, and AI conversations can each add context—but they arrive in different formats, with different evidence boundaries. Patients Intelligence creates a consistent way to inspect what arrived, separate sources from inference, surface gaps, and record what happened next." />
      ...
      <section className="section standard-home"><div className="container standard-grid"><div><div className="eyebrow-row"><span className="eyebrow">Open infrastructure</span><StatusTag status="draft" /></div><h2>Build the protocol before the category hardens without one.</h2><p>The PGI Standard defines a common structure for provenance, evidence labeling, urgency handling, acknowledgment, routing, human review, and audit metadata for patient-generated intelligence.</p><div className="button-group"><Link className="button button-primary" href="/pgi-standard">Explore the PGI framework</Link><Link className="button button-secondary" href="/partners#founding-100">Join the Founding 100</Link></div></div><div className="standard-record"><span className="document-kicker">PGI Standard / v0.9 reference model</span>{["Provenance", "Assertion boundaries", "Evidence classes", "Human review", "Disposition events", "Audit history"].map((x, i) => <div key={x}><span>0{i + 1}</span><strong>{x}</strong><small>{i < 2 ? "Required metadata" : "Shared framework"}</small></div>)}</div></div></section>
