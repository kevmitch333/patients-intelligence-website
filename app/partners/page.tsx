...
export default function Partners() {
  return <main id="main-content"><PageHero eyebrow="Partners" title="Help build the intake standard patient-generated AI now requires." lede="Choose a specific contribution: one bounded study, one implementation perspective, one research method, or one governance responsibility." />
    ...
    <section className="section founding-section" id="founding-100"><div className="container"><div className="eyebrow-row"><span className="eyebrow">Founding 100</span><StatusTag status="forming" /></div><SectionHeading title="A multidisciplinary working cohort—not a mailing list." copy="The cohort brings clinical, patient, research, and institutional authority into the standard before software claims outrun evidence." /><div className="founding-grid">{founding100.map(item => <article key={item.label}><strong>{item.count}</strong><h3>{item.label}</h3><p>{item.contribution}</p></article>)}</div><div className="cohort-exchange"><div><h3>Participants contribute</h3><p>Time, critique, implementation context, conflicts disclosure, and accountable review of shared materials.</p></div><div><h3>Participants receive</h3><p>Documented participation, early working-session access, and a meaningful role in shaping the open specification.</p></div></div><Link className="button button-primary" href="/contact?path=founding-100">Apply to participate <ArrowRight size={17} /></Link></div></section>
