...
export const metadata = { title: "PGI Standard", description: "An open shared structure for patient-generated intelligence, provenance, evidence labels, routing, and human review." };

const jsonConcept = `{
  "specification": "PGI v0.9 reference model",
  "status": "open_specification",
  "submission": {
...
export default function PGIStandard() {
  return <main id="main-content"><TrustStrip /><PageHero eyebrow="PGI Standard" title="A shared structure for patient-generated intelligence." lede="Patient-Generated Intelligence (PGI) is clinical analysis, synthesis, or interpretation produced by an AI system at a patient’s direction using patient-provided symptoms, records, device data, reports, questions, or cited sources as input." status="draft"><div className="button-group"><Link className="button button-primary" href="/contact?path=standard">Discuss the specification <ArrowRight size={17} /></Link><Link className="button button-secondary" href="/partners#founding-100">Join the collaboration</Link></div></PageHero>
    ...
    <section className="section"><div className="container participation"><SectionHeading eyebrow="Open participation" title="The specification benefits from implementation, clinical, patient, research, and governance perspectives." /><div>{[[FileText, "Test the framework", "Examine definitions, minimum fields, and failure states."], [Braces, "Contribute an implementation perspective", "Map the concept to portals, FHIR, provenance, and audit infrastructure."], [Clock3, "Submit a research question", "Strengthen validation, safety, burden, and equity methods."], [Users, "Join the Founding 100", "Help establish accountable, multi-stakeholder stewardship."]].map(([Icon, title, copy]) => { const I = Icon as typeof FileText; return <article key={String(title)}><I /><h3>{String(title)}</h3><p>{String(copy)}</p><Link href="/contact?path=standard">Start this conversation <ArrowRight size={15} /></Link></article>; })}</div></div></section>
    <section className="section glossary-section"><div className="container"><SectionHeading eyebrow="Shared glossary" title="Technical precision, plain-language explanations." /><dl>{glossary.map(([term, definition]) => <div key={term}><dt>{term}</dt><dd>{definition}</dd></div>)}</dl></div></section><CTASection title="Help define a protocol the category can share." copy="The PGI Standard is an open specification for review, testing, and responsible implementation." /></main>;
}
